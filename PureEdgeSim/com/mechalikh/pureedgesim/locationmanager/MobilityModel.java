...
public abstract class MobilityModel {
	...
	private ComputingNode getDataCenter(Location newLocation) {
		List<? extends DataCenter> list = getSimulationManager().getDataCentersManager().getEdgeDatacenterList();
		double range = SimulationParameters.EDGE_DATACENTERS_RANGE;
		ComputingNode closestDC = ComputingNode.NULL;
		for (int i = 0; i < list.size(); i++) {
			for(int j=0; j<list.get(i).nodeList.size(); j++) {
				if (list.get(i).nodeList.get(j).isPeripheral() && distanceTo(list.get(i).nodeList.get(j)) <= range) {
					range = distanceTo(list.get(i).nodeList.get(j));
					closestDC = list.get(i).nodeList.get(j);
				}
			}
		} 
		return closestDC;
