...
import applications.bqprofit.CostProfitCalculator;
...
public class SimulationManager extends SimulationManagerAbstract {
	...
	public boolean taskFailed(Task task, int phase) {
		...
		if (phase == 3 && task.getActualNetworkTime() + task.getWatingTime() + task.getActualCpuTime() > task
				.getMaxLatency()) {
			//double tot = task.getActualNetworkTime() + task.getWatingTime() + task.getActualCpuTime();
			//System.out.println("Task " + task.getId() + " failed for job " + task.getApplicationID() +  " Deadline: " + task.getMaxLatency() + " Total: " + tot + " Waiting: "+task.getWatingTime()+ " Nw: "+task.getActualNetworkTime() + " cpu: "+task.getActualCpuTime());
