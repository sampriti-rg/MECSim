...
public abstract class SimulationAbstract {
	 ...
	 */
	protected Class<? extends TaskGenerator> tasksGenerator = DefaultTasksGenerator.class;
	...
	public void setCustomSettingsFolder(String settingsFolder, String paramSettingFolder) {
		setCustomFilePath(paramSettingFolder + "simulation_parameters.properties", Files.SIMULATION_PARAMETERS);
		setCustomFilePath(settingsFolder + "applications.xml", Files.APPLICATIONS_FILE);
		setCustomFilePath(settingsFolder + "edge_datacenters.xml", Files.EDGE_DATACENTERS_FILE);
		setCustomFilePath(settingsFolder + "edge_devices.xml", Files.EDGE_DEVICES_FILE);
		setCustomFilePath(settingsFolder + "cloud.xml", Files.CLOUD_FILE);
		loadSpecificConfig(paramSettingFolder + "simulation_parameters.properties");
	}

	private void loadSpecificConfig(String path){
		...
		try {
			...
			String algorithms[] = prop.getProperty("orchestration_algorithms").split(",");
			GlobalConfig globalConfig = GlobalConfig.getInstance();
			globalConfig.setAlgorithm(algorithms[0]);

			boolean is_iot_dataset = Boolean.parseBoolean(prop.getProperty("iot_dataset").trim());
			if(is_iot_dataset) {
				String iot_dataset_task_path = prop.getProperty("iot_dataset_task_path");
				String iot_dataset_job_path = prop.getProperty("iot_dataset_job_path");
				globalConfig.setDatasetType(GlobalConfig.DatasetType.IOT_DATASET);
				globalConfig.setTaskSetPath(iot_dataset_task_path);
				globalConfig.setJobsPath(iot_dataset_job_path);
			}

			boolean is_scientific_workflow = Boolean.parseBoolean(prop.getProperty("scientific_workflow").trim());
			if(is_scientific_workflow){
				String scientific_workflow_dir =  prop.getProperty("scientific_workflow_dir");
				globalConfig.setDatasetType(GlobalConfig.DatasetType.SCIENTIFIC_WORKFLOW);
				globalConfig.setScientificWorkFlowDir(scientific_workflow_dir);
			}

			boolean is_synthetic_tasks = Boolean.parseBoolean(prop.getProperty("synthetic_tasks").trim());
			if(is_synthetic_tasks){
				String synthetic_tasks = prop.getProperty("synthetic_tasks_dir");
				globalConfig.setDatasetType(GlobalConfig.DatasetType.SYNTHETIC_TASKS);
				globalConfig.setSyntheticTasksDir(synthetic_tasks);
			}

			Integer max_apps =  Integer.parseInt(prop.getProperty("max_apps").trim());
			globalConfig.setMaxApps(max_apps);

			boolean modern_arch = Boolean.parseBoolean(prop.getProperty("modern_arch").trim());
			globalConfig.setModern_arch(modern_arch);
			input.close();
