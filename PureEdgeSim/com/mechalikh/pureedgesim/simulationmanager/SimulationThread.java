...
import applications.bqprofit.BQProfitSimMananger;
import applications.fptosm.FpTosmSimManager;
import applications.remec.ReMECSimManager;
import applications.rmeac.RmeacSimManager;
...
import applications.delta.DeltaSimMananger;
		...
		try {
			...
			for (int it = fromIteration; it < simulation.getScenarios().size(); it += step) {
				...
				if(GlobalConfig.getInstance().getAlgorithm().equals("DELTA")) {
					simulationManager = new DeltaSimMananger(simLog, pureEdgeSim, simulationId, iteration,
							simulation.getScenarios().get(it));
				} else if (GlobalConfig.getInstance().getAlgorithm().equals("BQProfit")) {
					simulationManager = new BQProfitSimMananger(simLog, pureEdgeSim, simulationId, iteration,
							simulation.getScenarios().get(it));
				} else if (GlobalConfig.getInstance().getAlgorithm().equals("ReMEC")) {
					simulationManager = new ReMECSimManager(simLog, pureEdgeSim, simulationId, iteration,
							simulation.getScenarios().get(it));
				} else if (GlobalConfig.getInstance().getAlgorithm().equals("FP-TOSM")) {
					simulationManager = new FpTosmSimManager(simLog, pureEdgeSim, simulationId, iteration,
							simulation.getScenarios().get(it));
				} else if (GlobalConfig.getInstance().getAlgorithm().equals("RMEAC")) {
					simulationManager = new RmeacSimManager(simLog, pureEdgeSim, simulationId, iteration,
							simulation.getScenarios().get(it));
				} else{
				...
				GlobalConfig.getInstance().setSimulationManager(simulationManager);

				// Load custom classes and models.
				loadModels(simulationManager);

				if(GlobalConfig.getInstance().getAlgorithm().equals("DELTA")) {
					((DeltaSimMananger) simulationManager).setPureEdgeSim(pureEdgeSim);
				} else if (GlobalConfig.getInstance().getAlgorithm().equals("BQProfit")) {
					((BQProfitSimMananger) simulationManager).setPureEdgeSim(pureEdgeSim);
				} else if (GlobalConfig.getInstance().getAlgorithm().equals("ReMEC")) {
					((ReMECSimManager) simulationManager).setPureEdgeSim(pureEdgeSim);
				} else if (GlobalConfig.getInstance().getAlgorithm().equals("FP-TOSM")) {
					((FpTosmSimManager) simulationManager).setPureEdgeSim(pureEdgeSim);
				} else if (GlobalConfig.getInstance().getAlgorithm().equals("RMEAC")) {
					((RmeacSimManager) simulationManager).setPureEdgeSim(pureEdgeSim);
				}
